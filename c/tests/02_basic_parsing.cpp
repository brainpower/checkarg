...
TEST_CASE("parsing: help using --help", "[parsing]") {
  ...
  CheckArgUPtr ca(checkarg_new(argv.size(), (char**)argv.data(), "test01", NULL, NULL), &checkarg_free);
...
TEST_CASE("parsing: correct option and value", "[parsing]"){
  ...
  REQUIRE(checkarg_isset(ca.get(), "input")); // option detected
  REQUIRE(checkarg_value(ca.get(), "input") == string("input.in")); // value stored correctly
}
